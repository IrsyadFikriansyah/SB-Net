...
test_path = 'Datasets/train-test.csv'

log_file = "logs/feature_n_results_max_depth_29.log"
...
for col_index in range(21, len(feature_cols)):
        log_message(f'Processing dataset: {train_path}')
        df_train = df_train_og.copy()
        df_train = df_train.drop(columns=feature_cols[col_index+1:], errors='ignore')
        log_message(str(len(df_train.columns)) + ' ' + ', '.join(list(df_train.columns)))
        ...
        for stage1_name, stage1_model in models.items():
            ...
            df_test = df_test.drop(columns=feature_cols[col_index+1:], errors='ignore')
            ...
            precision_bc = precision_score(y_test_stage1, y_pred_stage1, average='weighted')
            recall_bc = recall_score(y_test_stage1, y_pred_stage1, average='weighted')
            f1_bc = f1_score(y_test_stage1, y_pred_stage1, average='weighted')
            f2_bc = fbeta_score(y_test_stage1, y_pred_stage1, beta=2, average='weighted')
